...
trajectories, p = [75, 74.5, 74, 73.5, 73], 100
...
lec_spikes = lec_noise(n_cells=20, p_seeds=[args.grid_seed + 30] * 20)

# Create spikes
"""Setup"""
dirname = os.path.dirname(__file__)
results_dir = os.path.join(dirname, 'data', 'noise_lec_identical')
...
network_type = "no-feedback"  # "full", "no-feedback", "no-feedforward" or "disinhibited"
...
for grid_seed in grid_seeds:
    ...
    for traj in trajectories:
        ...
        for idx, poisson_seed in enumerate(poisson_seeds):
            print(f"Running trajectory: {traj} and poisson seed: {poisson_seed}")
