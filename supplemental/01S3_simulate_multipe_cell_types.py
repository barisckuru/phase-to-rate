...
poisson_seeds = np.arange(p, p + 10, 1)
poisson_seeds = list(poisson_seeds)

"""Parameters"""
shuffling = "non-shuffled"  # "non-shuffled" or "shuffled"
network_type = "disinhibited"  # "full", "no-feedback", "no-feedforward" or "disinhibited"
...
for grid_seed in grid_seeds:
    ...
    for traj in trajectories:
        ...
        for poisson_seed in poisson_seeds:
            all_spikes[traj][poisson_seed] = {}
            all_spikes_poiss = pydentate_integrate.granule_simulate_all_cell_types(
            ...
            all_spikes[traj][poisson_seed] = all_spikes_poiss
            print(f'pseed {poisson_seed} done.')

    storage = shelve.open(file_path, writeback=True)
    storage["grid_spikes"] = copy.deepcopy(grid_spikes)
    storage["all_spikes"] = copy.deepcopy(all_spikes)
...
plt.plot(t, granule_spikes[75][100].populations[0].VRecords[1])
plt.ylim((-100, 50))
plt.xlabel("Time (s)")
