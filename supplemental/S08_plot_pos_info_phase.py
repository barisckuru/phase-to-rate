...
fname = 'C:/phase-to-rate/pos_info_phase_non-adjusted.xlsx'
df = pd.read_excel(fname, index_col=0)
grid_seeds = list(np.arange(1,11,1))
grid_info = df[df['cell']=='full grid']
grid_info['grid_seed'] = 2*grid_seeds
granule_info = df[df['cell']!='full grid']
...
ax1.set_ylabel('phase info (bits/AP)')
...
f1.savefig(f'{save_dir}figure_pos_info_phase.svg', dpi=200)
#f1.savefig(f'{save_dir}figure_pso_info_phase.png', dpi=200)
...
#fname = results_dir + 'excel/pos_info_phase_results.xlsx'
fname = 'C:/phase-to-rate/pos_info_phase_non-adjusted.xlsx'
...
dfa3['scaled value (non-shuffled/shuffled)'] = dfa2_div
...
sns.boxplot(x='cell', y='scaled value (non-shuffled/shuffled)', ax=ax,
            ...
            hue='shuffling')
#sns.scatterplot(x='cell', y='scaled value (non-shuffled/shuffled)', ax=ax,
...
sns.lineplot(x='cell', y='scaled value (non-shuffled/shuffled)', ax=ax,
             zorder=1, legend=False,data=dfa3, linewidth=1,units='gridseed',
             estimator=None, color='black', alpha=0.3, errorbar=None)
#ax.set_ylim([1,1.4])
ax.set_xticklabels(['EC','full', 'no ff', 'no fb', 'disinh']
                    , rotation=60)
ax.set_title('non-shuffled/shuffled phase')
...
plt.rcParams["svg.fonttype"] = "none"
f2.savefig(f'{save_dir}figure_pos_info_phase_norm.svg', dpi=200)
#f2.savefig(f'{save_dir}figure_pos_info_phase.png', dpi=200)
