...
for grid_seed in grid_seeds:
    ...
    (
        ...
        grid_phase_code,
        grid_polar_code,
    ) = rate_n_phase(grid_spikes, trajectories, n_samples, bin_size_ms=2000)
    ...
    (
        ...
        granule_phase_code,
        granule_polar_code,
    ) = rate_n_phase(granule_spikes, trajectories, n_samples, bin_size_ms=2000)
    ...
    (
        ...
        s_grid_phase_code,
        s_grid_polar_code,
    ) = rate_n_phase(s_grid_spikes, trajectories, n_samples, bin_size_ms=2000)
    ...
    ) = rate_n_phase(s_granule_spikes, trajectories, n_samples, bin_size_ms=2000)
...
# pearsons R
# =============================================================================
pearsonr(granule_counts[:,0,0,0], granule_counts[:,0,0,16])


plt.figure()
plt.plot(grid_counts[:,0,0,0])
plt.plot(grid_counts[:,0,0,16])
            ...
            for code in all_codes[grid_seed][shuffling][cell]:
                for traj_idx in range(len(trajectories)):
                    ...
                    baseline_traj = np.concatenate((
                        r_input_code[1000*f:4000*f, 0, 0], 
                        r_input_code[5000*f:8000*f, 0, 0]))
                    for poisson in range(r_input_code.shape[2]):
                        compared_traj = r_input_code[:, poisson, traj_idx]
                        compared_traj = np.concatenate((
                            compared_traj[1000*f:4000*f],
                            compared_traj[5000*f:8000*f]))
                        pearson_r = pearsonr(baseline_traj, compared_traj)[0]
                        spearman_r = spearmanr(baseline_traj, compared_traj)[0]
                        traj = trajectories[traj_idx]
                        idx = 75 - traj
