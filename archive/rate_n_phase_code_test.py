...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 11 10:40:26 2021
...
def randomize_grid_spikes(arr, bin_size_ms, asig=asig, time_ms=2000):
    ...
    for i in range(n_bins):
        ...
        curr_train = stg.inhomogeneous_poisson_process(asig, as_array=True)*1000
...
def phase_code(trajs, dur_ms, grid_seed, poiss_seeds, pp_weight, f=10, shift_deg=180):
    ...
    for idx, poiss_seed in enumerate(poiss_seeds):
        curr_grid_spikes = inhom_poiss(overall_dir, n_traj, dur_s, shuffle=True, dt_s=dt_s, poiss_seed=poiss_seed)
        
        print(curr_grid_spikes.shape)
        # curr_grid_spikes = randomize_grid_spikes(act_grid_spikes, bin_size_ms, dt_ms=25, time_ms=dur_ms)
        # print(curr_grid_spikes)
        # curr_gra_spikes = pyDentate(curr_grid_spikes, trajs, grid_seed, poiss_seed, n_traj, dur_ms, pp_weight)[0]
        curr_gra_spikes = inhom_poiss(overall_dir, n_traj, dur_s, shuffle=False, dt_s=dt_s, poiss_seed=poiss_seed)
        #grid phases
...
def phase_code_diff_poiss(trajs, dur_ms, grid_seed, poiss_seeds, pp_weight, f=10, shift_deg=180):
    dur_s = dur_ms/1000
    T = 1/f
    bin_size_ms = T*1000
    n_phase_bins = 360
    ...
    gra_spikes = np.zeros(len(poiss_seeds), dtype = np.ndarray)
    for idx, poiss_seed in enumerate(poiss_seeds):
        curr_grid_spikes = inhom_poiss_diff(overall_dir, n_traj, dur_s, dt_s=dt_s, poiss_seed=poiss_seed)
...
def pyDentate(input_grid_out, trajs, grid_seed, poiss_seed, n_traj, dur_ms, pp_weight):
    ...
    for trj in range(n_traj):
        ...
        tuned_fig_file_name = (str(nw) + "spike_plot_rate_nofb_n_phase_gra_out_for_perceptron_seed3_dur_weight_"+
    ...
    fname = 'pyDentate_nofb_out_traj_'+str(trajs[0])+'-'+str(trajs[1])+'_dur_'+str(dur_ms)+'_weight_'+str(pp_weight)+'_seed1_'+str(grid_seed)+'_seed2_'+str(poiss_seed)
    np.savez(path+fname, 
