...
binary_spikes_full = np.zeros((spikes_full.shape[0], 20000))
for idx, cell in enumerate(spikes_full):
    spike_idc = np.array(cell * 10, dtype=int)
    binary_spikes_full[idx, spike_idc] = 1
    
binary_spikes_nofb = np.zeros((spikes_nofb.shape[0], 20000))
for idx, cell in enumerate(spikes_nofb):
    spike_idc = np.array(cell * 10, dtype=int)
    binary_spikes_nofb[idx, spike_idc] = 1


"""Apply a gaussian filter to the spike trains"""

gaussian_kernel = scipy.signal.windows.gaussian(1000,100)
binary_spikes_full_convolved = np.zeros((spikes_full.shape[0], 20000))
for idx, cell in enumerate(binary_spikes_full):
    curr_conv = scipy.signal.convolve(cell, gaussian_kernel, mode='same')
    if not curr_conv.sum() == 0:
        binary_spikes_full_convolved[idx] = curr_conv / curr_conv.sum()
    else:
        binary_spikes_full_convolved[idx] = 0
    
binary_spikes_nofb_convolved = np.zeros((spikes_nofb.shape[0], 20000))
for idx, cell in enumerate(binary_spikes_nofb):
    curr_conv = scipy.signal.convolve(cell, gaussian_kernel, mode='same')
    if not curr_conv.sum() == 0:
        binary_spikes_nofb_convolved[idx] = curr_conv / curr_conv.sum()
    else:
        binary_spikes_nofb_convolved[idx] = 0

"""Calculate density for the different conditions"""
spikes_full_nofb_density = copy.deepcopy(spikes_full)
spikes_full_full_density = copy.deepcopy(spikes_full)
spikes_nofb_nofb_density = copy.deepcopy(spikes_nofb)
spikes_nofb_full_density = copy.deepcopy(spikes_nofb)

for cell_idx, cell in enumerate(spikes_full):
    for spike_idx, spike in enumerate(cell):
        spike_binary_index = int(spike * 10)
        spikes_full_nofb_density[cell_idx][spike_idx] = binary_spikes_nofb_convolved[cell_idx, spike_binary_index]
        spikes_full_full_density[cell_idx][spike_idx] = binary_spikes_full_convolved[cell_idx, spike_binary_index]

for cell_idx, cell in enumerate(spikes_nofb):
    for spike_idx, spike in enumerate(cell):
        spike_binary_index = int(spike * 10)
        spikes_nofb_nofb_density[cell_idx][spike_idx] = binary_spikes_nofb_convolved[cell_idx, spike_binary_index]
        spikes_nofb_full_density[cell_idx][spike_idx] = binary_spikes_full_convolved[cell_idx, spike_binary_index]

spikes_full_nofb_density_flat = np.array([x for cell in spikes_full_nofb_density for x in cell])
spikes_full_full_density_flat = np.array([x for cell in spikes_full_full_density for x in cell])
spikes_nofb_nofb_density_flat = np.array([x for cell in spikes_nofb_nofb_density for x in cell])
spikes_nofb_full_density_flat = np.array([x for cell in spikes_nofb_full_density for x in cell])
...
spikes_full_phases_flat = [x for cell in spikes_full_phases for x in cell]
spikes_nofb_phases_flat = [x for cell in spikes_nofb_phases for x in cell]

"""Find the 100 most active cells across poisson seeds"""
n_max_cells = 50
total_spikes_nofb = np.array([len(spikes) for spikes in spikes_nofb])
most_active = np.argpartition(total_spikes_nofb, -n_max_cells)[-n_max_cells:]
