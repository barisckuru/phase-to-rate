...
sim_traj = np.array([71, 70])
diff_traj = np.array([65,60])
...
grid_phases_sim,  gra_phases_sim, grid_spikes_sim, ori_grid_spikes_sim, gra_spikes_sim, rate_trajs_sim, dt_s, theta_phase_sim, phase_code_dir_sim, overall_dir_sim, traj_dist_dir_sim, dist_trajs_sim, direction_sim, spacing_sim, grids_sim = phase_code(sim_traj, dur_ms, grid_seeds, poiss_seeds, pp_weight)
grid_phases_diff, gra_phases_diff, grid_spikes_diff, ori_grid_spikes_diff, gra_spikes_diff, rate_trajs_diff, dt_s, theta_phase_diff, phase_code_dir_diff, overall_dir_diff, traj_dist_dir_diff, dist_trajs_diff, direction_diff, spacing_diff, grids_diff = phase_code(diff_traj, dur_ms, grid_seeds, poiss_seeds, pp_weight)
...
save_dir = '/home/baris/results/grid_mixed_input/full/71-70-65-60/non-shuffled/'
fname = 'non-shuffled_codes_full_traj_'+str(sim_traj[0])+'-'+str(sim_traj[1])+'-'+str(diff_traj[0])+'-'+str(diff_traj[1])+'_net-seed'+str(perc_seeds)+'_'+str(dur_ms)+'ms'
# fname = "test"
