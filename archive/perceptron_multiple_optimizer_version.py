...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov  1 21:10:12 2021
...
def _train_net(net, train_data, labels, n_iter=1000, optimizer='SGD', lr=1e-4):
    ...
    if optimizer == 'SGD':
        ...
        optimizer = optim.SparseAdam(list(net.parameters()), lr=lr)
    elif optimizer == 'Adagrad':
        optimizer = optim.SparseAdam(list(net.parameters()), lr=lr)
    else:
        raise Exception('Given optimizer does not exist')
    track_loss = []
    loss_fn = nn.MSELoss()
    # loss_fn = nn.L1Loss()
...
def run_perceptron(neural_code, grid_seed, optimizer='SGD', learning_rate=1e-4,
    ...
    train_loss, _ = _train_net(net_neural, neural_code,
                               labels, optimizer=optimizer, n_iter=n_iter,
                               lr=learning_rate)
