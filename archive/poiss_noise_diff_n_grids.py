...
'''poisson noise for variable cell number for grid cells'''
...
cell_numbers = np.array([50,100,200,300,400])
par_trajs_pf = []


for n_grid in cell_numbers:
    print(n_grid)
...
counts = np.empty((cell_numbers.size, seed_2s.size), dtype=np.ndarray)
...
plt.legend(cell_numbers[arr], title='num of grids')
        
parameters = ('bin size = 500 ms      parallel trajs = '+str(par_trajs)+' cm      max rate = '+str(max_rate)+
