...
for tuning in tunings:
    ...
    for idx, cell in enumerate(spikes_full[tuning]):
        ...
        curr_binary_spikes[idx, spike_idc] = 1
    ...
    for density_condition in tunings:
        ...
        for cell_idx, cell in enumerate(spikes_full[spike_condition]):
            for spike_idx, spike in enumerate(cell):
                ...
                curr_spike_density[cell_idx][spike_idx] = binary_spikes_convolved[density_condition][cell_idx, spike_binary_index]
...
spikes_phases_flat = {}
for tuning in tunings:
    curr_phases = [((np.array(x) % cycle_ms) / cycle_ms) * 2 * np.pi for x in spikes_full[tuning]]
    spikes_phases[tuning] = curr_phases
    spikes_phases_flat[tuning] = [x for cell in curr_phases for x in cell]

"""Find the 100 most active cells across poisson seeds"""
n_max_cells = 50
total_spikes_nofb = np.array([len(spikes) for spikes in spikes_full['disinhibited']])
...
eventplot_collection =  ax4.eventplot([spikes_full['disinhibited'][example_cell_idx],
                                       spikes_full['no-feedback'][example_cell_idx],
                                       spikes_full['no-feedforward'][example_cell_idx],
                                       spikes_full['full'][example_cell_idx]],
                                       color='k')
"""
...
ax5.plot(binary_spikes_convolved['full'][example_cell_idx])
ax5.plot(binary_spikes_convolved['no-feedback'][example_cell_idx])
ax5.plot(binary_spikes_convolved['no-feedforward'][example_cell_idx])
ax5.plot(binary_spikes_convolved['disinhibited'][example_cell_idx])
ax5.legend(("full", "no-feedback", "no-feedforward", "disinhibited"))
